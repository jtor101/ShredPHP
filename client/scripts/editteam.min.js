...
$("#teamSubmitBtn").on("click",sendForm);function sendForm(){let isOk=inputVal();if(isOk==!1){return}else{confirm("Are you sure you want to confirm these changes?");$.ajax({url:"http://localhost:8081/api/teams/",method:"PUT",dataType:"json",data:$("#editTeam").serialize()});alert("Band Info updated!")}}
$.getJSON("http://localhost:8081/api/teams/"+id,function(data){let team=data;$("#teamId").val(team.TeamId);$("#teamName").val(team.TeamName);$("#teamLeague").val(team.League);$("#managerName").val(team.ManagerName);$("#managerPhone").val(team.ManagerPhone);$("#emailAddr").val(team.ManagerEmail);$("#maxTeamMembers").val(team.MaxTeamMembers);$("#minAge").val(team.MinMemberAge);$("#maxAge").val(team.MaxMemberAge);$("#teamGender").val(team.TeamGender)})})