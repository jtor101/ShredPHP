...
$(function() {
  ...
  function catFillCard() {
    $.getJSON(
      "/http://localhost:8081/api/teams/byleague/" + $("#divisionSel").val(),
      function(data) {
        let teams = data;
        ...
        for (let i = 0; i < teams.length; i++) {
          ...
          let editTeamLink = $(
            "<a class='btn btn-secondary text-light' href='editteam.php?teamid=" +
              teams[i].TeamId +
              "'>Edit Band Details</a>"
          );
          // Generates View Roster Details Buttons and assigns unique id and URL.
          let detailsLink = $(
            "<a class='btn btn-dark text-light mx-2' href='teamdetails.php?teamid=" +
              teams[i].TeamId +
          ...
          $("#team" + teams[i].TeamId).on("click", function() {
            ...
            $.ajax({
              url: "/http://localhost:8081/api/teams/" + teams[i].TeamId,
              method: "DELETE",
              success: function() {
                alert("Updated!");
              }
